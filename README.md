...
- 重新设计了非RT模式下的静态CrossView算法流程：

##### 一、世界坐标系建立

- 读入file1,file2，获得如下信息：图像左上角的世界坐标，图像横纵方向的方向向量，图像的法向量（横方向向纵方向的右手叉乘），图像的行数列数，图像的像素间距

##### 二、计算交线的三维参数方程

- 设交线上一点为`P =（x,y,z）`，两图像的左上角分别为`O1,O2`，各自的法向量为`n1,n2`
...
- 利用sympy的符号计算库，可以得到P关于x,y,z的参数方程，且结果中有且仅有一个未知数
- 将计算结果除以图像的像素间距，可得到该点相对于图像行数列数的数值

##### 三、获得交线上的两个定点

- 取x,y,z = (0,0,0)和x,y,z=(1,1,1)，获得交线上的两个定点的世界坐标。这两个坐标必定在两幅图像上
- 设两个定点为P1，P2，则对于O1所在的图1来说，以O1为原点，以横纵方向为x,y轴，可以建立二维坐标系，设坐标轴为x1,y1
- P1，P2是坐标系x1-y1上的两个定点，必可以用x1,y1建立形如 L : `mx1+ny1+b=0`的直线方程

##### 四、获得交线与图像边界的两个交点

- 假设图1行数300，列数200，则在x1-y1坐标系下，其图像的四个边界的直线方程可表示为

$$
x=0\space\space y=0\space\space x=300\space\space y=200 
$$

- 依次将直线L与边界方程联立，可以解出4个交点坐标（排除平行的特殊情况）
- 这四个交点坐标中必有两个交点的坐标值不在 0<x<300和0<y<200范围内，排除这两点

##### 五、自定义QT View Painter渲染crossView

- 获得两个交点后，在qt paintEvent事件中，绘制连接这两点的矩形框
...
- 遇到主页面在跑马灯效果下卡顿问题，问题未解决。已经尝试的思路：
  - Qthread优化：无效
  - 减少Qpainter绘制的数量：无效
- 毫无思路